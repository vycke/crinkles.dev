...
<Page class="stack | mb-3 text-00" headerTitle={status}>
	...
	<span class="text-grey-2 mb-2 text-0 text-center">
	...
	<aside class="stack gap-0">
		{#each articles as article}
			<Card>
				<span class="text-00 text-grey-0 uppercase">
					{article.formattedDate}
				</span>
				<h2 class="text-0 text-grey-0">
					<a class="text-0 text-grey-0" href="/writing/{article.slug}">{article.title}</a>
				</h2>
			</Card>
		{/each}
	</aside>
